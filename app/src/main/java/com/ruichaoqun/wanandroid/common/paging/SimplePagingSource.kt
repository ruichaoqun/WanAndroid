...
abstract class SimplePagingSource<T,R>:PagingSource<Int,LoadMoreDataBean<R>>(){
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, LoadMoreDataBean<R>> {
        ...
        try {
            ...
            return LoadResult.Page(data = transFormData(list,position,showNoMoreItem(list,position,response)),prevKey = preKey,nextKey = nextKey)
    ...
    private fun transFormData(repos: List<R>?, position:Int,showNoMoreItem: Boolean): List<LoadMoreDataBean<R>> {
        if(repos.isNullOrEmpty() && position == INIT_PAGE_NUMBER){
            return arrayListOf(LoadMoreDataBean(null,2))
        }
        val result:ArrayList<LoadMoreDataBean<R>> = arrayListOf()
        for (data in repos?: mutableListOf()){
